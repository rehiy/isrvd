...
推荐按 **Python → Node.js → Bash** 的顺序选择脚本；三者共用 `~/.config/isrvd/profile.json`。
...
脚本默认提取统一响应中的 `.payload`；数组对象会自动转为紧凑表格，降低输出噪音。Python/Node 版支持简单 selector（如 `.content`、`.username`）；Bash 版支持 jq filter。
