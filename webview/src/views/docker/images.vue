...
// 查看镜像详情
const openInspectModal = async (image) => {
  inspectData.value = null
  inspectOpen.value = true
  inspectLoading.value = true
  try {
    const res = await api.imageInspect(image.id)
...
<button @click="openBuildModal()" class="px-3 py-1.5 rounded-lg bg-blue-500 hover:bg-blue-600 text-white text-xs font-medium flex items-center gap-1.5 transition-colors">
    ...
    <!-- Image Table -->
    <div v-else-if="images.length > 0" class="overflow-x-auto">
          ...
          <tr v-for="img in images" :key="img.id" class="hover:bg-slate-50 transition-colors">
            ...
            <td class="px-4 py-3">
              <div class="flex justify-center items-center gap-0.5">
                <button @click="openInspectModal(img)" class="btn-icon text-slate-600 hover:bg-slate-100" title="查看详情">
    ...
    <BaseModal
      ...
      size="lg"
    >
      <div v-if="inspectLoading" class="flex flex-col items-center justify-center py-10">
        <div class="w-10 h-10 spinner mb-3"></div>
        <p class="text-slate-500 text-sm">加载中...</p>
      </div>
      <div v-else-if="inspectData" class="space-y-4 text-sm">
        <!-- 基本信息 -->
        <div class="grid grid-cols-2 gap-3">
          ...
          <div class="col-span-2">
            ...
            <div class="px-3 py-2 bg-slate-50 rounded-lg text-slate-700">{{ inspectData.created }}</div>
          </div>
        ...
        <div v-if="inspectData.repoTags && inspectData.repoTags.length > 0">
          <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">标签</label>
        ...
        <div v-if="inspectData.repoDigests && inspectData.repoDigests.length > 0">
          <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">Digest</label>
          <div class="space-y-1">
        ...
        <div class="border-t border-slate-100 pt-4">
          <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-3">运行配置</label>
          <div class="space-y-3">
        ...
        <div v-if="inspectData.env && inspectData.env.length > 0">
          <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">环境变量</label>
        ...
        <div v-if="inspectData.labels && Object.keys(inspectData.labels).length > 0">
          <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">Labels</label>
          <div class="max-h-40 overflow-y-auto border border-slate-200 rounded-lg divide-y divide-slate-100">
