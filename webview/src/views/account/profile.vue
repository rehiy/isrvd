...
class Profile extends Vue {
    ...
    passkeyBeginData: any = null  // 预先缓存的注册 begin 数据
    ...
    async handleRegisterPasskey() {
        ...
        if (!this.passkeyBeginData) {
            this.portal.showNotification('error', '注册数据未就绪，请关闭后重试')
            return
        }
        ...
        try {
            ...
            // 将 options 中的 base64url 字段转为 ArrayBuffer（WebAuthn API 要求）
            ...
            // 直接调用 WebAuthn API（在用户手势的同步调用链中，Bitwarden 等扩展才会弹出）
    ...
    // 打开注册弹框时预先获取 begin 数据，确保后续 navigator.credentials.create() 在用户手势链中调用
